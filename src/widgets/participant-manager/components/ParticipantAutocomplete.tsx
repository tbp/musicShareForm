...
export const ParticipantAutocomplete = React.memo(function ParticipantAutocomplete({
  value,
  onChange,
  onSelectParticipant,
  onEditParticipant,
  ...
  const suggestions = useMemo((): ParticipantSuggestion[] => {
    if (value.length >= 2) {
      // Используем функцию поиска из утилит виджета
      const searchResult = searchParticipants([], value.trim().toLowerCase(), { limit: 10 })
